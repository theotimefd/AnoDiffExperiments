...
def launch_compute_metrics_reconstruction(args):
    ...
    # make sure its using the same brains for adc, t2 flair & t1
    ...
    for imagename in test_imagenames:
        test_images_adc.append(ROOT_DIR+f"datasets/final_adc_dataset_small/Dallas_registered/"+imagename+"_ADC.nii.gz")
        test_images_flair.append(ROOT_DIR+f"datasets/final_flair_dataset_small/dallas_registered/"+imagename+"_acq-FLAIR_run-1_T2w.nii.gz")
        test_images_t1.append(ROOT_DIR+f"datasets/final_t1_dataset_small/dallas_registered/"+imagename+"_acq-MPRAGE_run-1_T1w.nii.gz")

    test_reconstruction_datalist = [
        {"adc": adc_path, "flair": flair_path, "t1": t1_path}
        for adc_path, flair_path, t1_path in zip(test_images_adc, test_images_flair, test_images_t1)
    ]
    ...
    # transforms
    test_reconstruction_transforms_def = define_instance(args, "val_transforms")

    test_reconstruction_transforms = transforms.Compose([
        *test_reconstruction_transforms_def.transforms,
    ...
    for ch in range(min(3, test_reconstruction_images.shape[1])):

        for idx in range(min(4, test_reconstruction_images.shape[0])):
            channel_names = ['ADC', 'FLAIR', 'T1W']

            
            # Original test_reconstruction images
            original_image = test_reconstruction_images[idx, ch].cpu().numpy()
            axes[0, idx*2].imshow(original_image, cmap='gray', vmin=0, vmax=1)
            axes[0, idx*2].set_title(f'Original {idx+1} ({channel_names[ch]})')
            ...
            first_noisy_image_no_background = first_noisy_images[idx, ch].cpu().numpy().copy()
            ...
            axes[1, idx*2].set_title(f'Noisy {idx+1} ({channel_names[ch]}), {args.noise["noise_rate_visualize"]*100}% noise (timesteps={infer_timesteps_visualize})')
            ...
            # Inferred images
            infered_image = infered[idx, ch].cpu().numpy()
            axes[2, idx*2].imshow(infered_image, cmap='gray', vmin=0, vmax=1)
            axes[2, idx*2].set_title(f'Inferred {idx+1} ({channel_names[ch]})')
            ...
            axes[3, idx*2].set_title(f'Difference {idx+1} ({channel_names[ch]})')
            ...
            true = test_reconstruction_images[idx, ch].unsqueeze(0).unsqueeze(0)  # Add batch and channel dimensions
            ...
            #pred = infered[idx, ch].unsqueeze(0).unsqueeze(0)  # Add batch and channel dimensions   
        ...
        plt.savefig(f"{ROOT_DIR}/AnoDiffExperiments/{EXPERIMENT_NAME}/{SUB_EXPERIMENT_NAME}/{SUB_EXPERIMENT_NAME}_metrics_reconstruction{channel_names[ch]}.png", transparent=False, dpi=150)
