...
filelist_dallas_adc = [final_adc_dataset_dallas + item for item in filelist_dallas_adc if '_'.join(item.split('_')[:2]) in final_filelist_names_intersection]



random.shuffle(filelist_dallas_adc)


cutoff = int(0.8 * len(filelist_dallas_adc))
train_sublist = filelist_dallas_adc[:cutoff]

val_sublist = filelist_dallas_adc[cutoff:cutoff + (len(filelist_dallas_adc)-cutoff) // 2]

test_sublist = filelist_dallas_adc[cutoff + (len(filelist_dallas_adc)-cutoff) // 2:]
