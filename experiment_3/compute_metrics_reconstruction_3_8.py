...
model_path = os.path.join(MODELS_DIR, f"{EXPERIMENT_NAME}_best_model.pth")
...
train_loader = DataLoader(
    train_ds, batch_size=batch_size, shuffle=False, num_workers=num_workers, persistent_workers=True
)
...
for image_batch in tqdm(train_loader): #TODO for now this works only if there are less images than batch size

    train_images = image_batch.to(device)
    ...
    with autocast(device_type=DEVICE_TYPE, enabled=True):
        ...
        for i, noise_timesteps in enumerate(NOISE_RANGE):
            ...
            infered, first_noisy_images = my_sample(train_images, timesteps=noise_timesteps, progress_bar=False, return_first_noisy_image=True)


            mse[noise_timesteps].append(F.mse_loss(infered, train_images).detach().cpu().numpy().flatten())
            ssim[noise_timesteps].append(np.mean(ssim_metric(train_images, infered).detach().cpu().numpy().flatten()))
            psnr[noise_timesteps].append(psnr_metric(infered, train_images).detach().cpu().numpy().flatten())
...
for i,(image_batch) in enumerate(train_loader):
    ...
    with autocast(device_type=DEVICE_TYPE, enabled=True):
        infered_images = []
        infered, first_noisy_images = my_sample(train_images, timesteps=infer_timesteps, progress_bar=False, return_first_noisy_image=True)

# ----------- PLOT -----------

metric_result_text = f"With ({NOISE_MIN},{NOISE_MAX}) timesteps noise range, on the whole train_reconstruction_dataset (n={batch_size})\n"
...
for idx in range(min(4, train_images.shape[0])):

    # Original train images
    original_image = train_images[idx, 0].cpu().numpy()
    ...
    true = train_images[idx, 0].unsqueeze(0).unsqueeze(0)  # Add batch and channel dimensions
