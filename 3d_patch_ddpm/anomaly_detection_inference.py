...
def launch_anomaly_detection_inference(args, no_abs_value=False):
    ...
    if args.dataset["test"] == "soop":
        ...
        if no_abs_value:
            ...
            make_anomaly_maps_optim(args, model, device, infer_scheduler=infer_scheduler, image_loader=test_anomaly_large_loader_metrics, image_paths=test_anomaly_large_images[len(test_anomaly_large_images)//2:], infer_timesteps=best_num_timesteps_large_group, output_folder=ANOMALY_MAPS_DIR+"large_no_abs_value/", replace_existing_files=False, no_abs_value=no_abs_value)
        else:
