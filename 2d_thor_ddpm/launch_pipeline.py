...
from train_thor_ddpm import launch_train
...
def main():
    parser = argparse.ArgumentParser(description="2D THOR DDPM training script")
    parser.add_argument(
        "-c",
        "--config-file",
    ...
    for step in args.pipeline:
        ...
        if step == "train_ddpm":
            print(f"Launching thor ddpm training: {config_dict['experiment_name']}/{config_dict['sub_experiment_name']} with {args.gpus} gpus")
            launch_train(args)
