...
def process_volume(idx, test_images, image_paths, i, infer_patch_size, patch_overlap, patch_infer_batch_size, args, simplexObj, model, infer_scheduler, infer_timesteps, device, output_folder, basic_affine, replace_existing_files):
    ...
    reconstructed_map_path = ANOMALY_MAPS_DIR+f"ano_map_{image_name}"

    if os.path.exists(reconstructed_map_path):
        tprint(f"Anomaly map for patient {image_name} at noise timesteps {infer_timesteps} already exists, skipping inference.")
        return

    volume = test_images[idx : idx + 1]
    ...
    if median_filter_size is not None and median_filter_size > 0:
        ...
        final_anomaly_map_np = median_filter(final_anomaly_map_np, size=median_filter_size)
...
def compute_metrics(args, model, device, ANOMALY_MAPS_DIR, infer_scheduler, image_loader, image_paths, mask_loader, infer_timesteps, threshold, median_filter_size, erosion_dilation_iterations):
    ...
    if not no_masks:
        ...
        for i,(image_batch, mask_batch) in enumerate(tqdm(zip(image_loader, mask_loader))):
            ...
            ano_segmentation = infered_batch > threshold
...
def launch_compute_metrics_anomaly_detection(args):
    ...
    ano_batch_size = args.dataset["batch_size"]
