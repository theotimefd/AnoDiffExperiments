...
anomaly_maps_adc_dir = ROOT_DIR + f"datasets/anomaly_maps/exp_2_2_fixed/{current_group}/" 
anomaly_maps_flair_dir = ROOT_DIR + f"datasets/anomaly_maps/exp_3_2_fixed/{current_group}/" 

masks_dir = ROOT_DIR + f"datasets/final_soop_dataset_small/masks_combined_registered/" 



segmentation_1x_no_post_proc_folder = ROOT_DIR + f"datasets/segmentations/combined/segmentation_1x_no_post_proc/{current_group}/"
os.makedirs(segmentation_1x_no_post_proc_folder, exist_ok=True)
segmentation_1x_with_post_proc_folder = ROOT_DIR + f"datasets/segmentations/combined/segmentation_1x_with_post_proc/{current_group}/"
...
def launch_compute():
    ...
    for adc_ano_file in tqdm(os.listdir(anomaly_maps_adc_dir)):
        
        patient_nb = adc_ano_file.split(".")[0]

        scores_patients[patient_nb] = {}

        ano_map_adc = nib.load(os.path.join(anomaly_maps_adc_dir, adc_ano_file)).get_fdata()
        ano_map_flair = nib.load(os.path.join(anomaly_maps_flair_dir, adc_ano_file)).get_fdata()
        combined_ano_map = (ano_map_adc + ano_map_flair) / 2

        mask = nib.load(os.path.join(masks_dir, adc_ano_file)).get_fdata()

        # make sure the images are in shape (1, 1, H, W, D) for the scores function
        if len(combined_ano_map.shape) == 3:
            combined_ano_map = torch.from_numpy(combined_ano_map).unsqueeze(0).unsqueeze(0).to(device)
        if len(mask.shape) == 3:
            mask = torch.from_numpy(mask).unsqueeze(0).unsqueeze(0).to(device)

        # no post processing
        segmentation_no_post_proc = combined_ano_map > params[current_contrast][current_group]["1x"]["threshold"]
        ...
        # apply median filter
        anomaly_map_np = combined_ano_map.cpu().numpy()
        if params[current_contrast][current_group]["1x"]["median_filter_size"] > 0:
