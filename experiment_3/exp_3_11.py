...
def compute_loss(images, model, inferer, num_train_timesteps, device):
...
def main():
    ...
    [
        ...
        transforms.RandAffine(prob=0.5, rotate_range=(0.10, 0.10, 0.10), translate_range=(5, 5, 5)),#+- 0.15 radians for each axis
        ...
        custom_transforms.Get2DSliceWithRandomOffset(axis=2, fixed_offset=0, range_offset=20),
    ...
    for epoch in range(max_epochs):
        ...
        for step, batch in enumerate(train_loader):
            ...
            loss = compute_loss(images, model, inferer, num_train_timesteps, device)
        ...
        if (epoch + 1) % val_interval == 0:
            ...
            for step, batch in enumerate(val_loader):
                images = batch.to(device)
                val_loss = compute_loss(images, model, inferer, num_train_timesteps, device)
